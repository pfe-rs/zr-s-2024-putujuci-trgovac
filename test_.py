    ...
    for i in range(1, n + 1):
        for j in range(1, i):
            if i - j == 1 or i - j == n - 1:
                ...
                edges[j][i] = edges[i][j]
    print(edges)
    assert Solver.brute_force(edges, list(range(1, n + 1))) == (list(range(1, n + 1)), n) or Solver.nearestNeighbor(graph) == (list(range(1, n + 1 )), n) or Solver.simulated_annealing(edges,list(range(1, n + 1))) == (list(range(1, n + 1)), n)